        ...
        if (window.WebGLRenderingContext != null) {
            ...
            WebGLRenderingContext.prototype.getSupportedExtensions =
                function () {
                    ...
                    for (
                        _k = 0, _len2 = shimExtensions.length;
                        _k < _len2;
                        _k++
                    ) {
            ...
            return (WebGLRenderingContext.prototype.getFloatExtension =
                function (spec) {
                    ...
                    singleFramebuffer = this.getExtension(
    ...
    Heights = (function () {
        function Heights(heatmap, gl, width, height) {
            ...
            this.shader = new Shader(this.gl, {
                ...
                fragment:
                    "#ifdef GL_FRAGMENT_PRECISION_HIGH\n    precision highp int;\n    precision highp float;\n#else\n    precision mediump int;\n    precision mediump float;\n#endif\nvarying vec2 off, dim;\nvarying float vIntensity;\nvoid main(){\n    float falloff = (1.0 - smoothstep(0.0, 1.0, length(off/dim)));\n    float intensity = falloff*vIntensity;\n    gl_FragColor = vec4(intensity, falloff, 0.0, 1.0);\n}",
            });
        ...
        function WebGLHeatmap(_arg) {
            ...
            if (gradientTexture) {
                ...
                if (typeof gradientTexture === "string") {
                    ...
                    if (
                        gradientTexture.width > 0 &&
                        gradientTexture.height > 0
                    ) {
            ...
            if (intensityToAlpha) {
                ((_ref1 = alphaRange != null ? alphaRange : [0, 1]),
                    ...
                    (alphaEnd = _ref1[1]));
                output =
                    "vec4 alphaFun(vec3 color, float intensity, float shape){\n    float alpha = smoothstep(0.0, 0.1, shape) * 0.625 + smoothstep(0.0, 1.0, intensity) * 0.375;\n    return vec4(color*alpha, alpha);\n}";
            } else {
                output =
            ...
            this.shader = new Shader(this.gl, {
                ...
                fragment:
                    ...
                    ("float linstep(float low, float high, float value){\n    return clamp((value-low)/(high-low), 0.0, 1.0);\n}\n\nfloat fade(float low, float high, float value){\n    float mid = (low+high)*0.5;\n    float range = (high-low)*0.5;\n    float x = 1.0 - clamp(abs(mid-value)/range, 0.0, 1.0);\n    return smoothstep(0.0, 1.0, x);\n}\n\n" +
                        ...
                        "\n" +
                        output +
                        "\n\nvoid main(){\n    vec4 tex = texture2D(source, texcoord);\n    float intensity = smoothstep(0.0, 1.0, tex.r);\n    float shape = tex.g;\n    vec3 color = getColor(intensity);\n    gl_FragColor = alphaFun(color, intensity, shape);\n}"),
            });
            if (this.width == null) {
